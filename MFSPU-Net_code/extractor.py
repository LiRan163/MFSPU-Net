...
def load_weights_sequential(target, source_state):
    ...
    for source_key, source_param in source_state.items():
        if "fc." in source_key:
            continue
        if source_key in target_state and source_param.shape == target_state[source_key].shape:
            new_dict[source_key] = source_param
            # print(f"Successfully loaded: {source_key}")
        else:
            print(f"Skipped (mismatch): {source_key} (source shape: {source_param.shape}, target shape: {target_state.get(source_key, 'Not Found').shape if source_key in target_state else 'Not Found'})")
    target_state.update(new_dict)
